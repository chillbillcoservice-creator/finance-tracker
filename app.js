...
function renderDashboard() {
  ...
  if (rentalBalanceNode) {
    if (isDayMode || isYearMode) {
      ...
      rentalBalanceNode.innerHTML = `<span style="color: var(--text-primary);">Collected: <span style="color: var(--color-warning); font-weight: 800; margin-left: 2px;">${formatCurrency(totalRentCollected)}</span></span> <span style="color: var(--text-primary);">Monthly Income: <span style="color: var(--text-primary); font-weight: 800; margin-left: 2px;">${formatCurrency(monthlyRent)}</span></span>`;
    }
  }
