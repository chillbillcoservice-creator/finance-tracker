...
function renderLending() {
  ...
  loanGroups.forEach(function(group) {
    ...
    group.loans.forEach(function(l) { groupOutstanding += l._stats.outstandingPrincipal; groupYield += l._stats.monthlyYield; });
    ...
    // Header
    var hdrHtml = '<div style="font-weight:700;font-size:1rem;">' + group.label + '</div>';
    if (first.phone) hdrHtml += '<div style="font-size:0.82rem;color:#fff;margin-top:0.05rem;">' + first.phone + '</div>';
    hdrHtml += '</div><div style="text-align:right;"><div style="font-size:1.15rem;font-weight:800;color:var(--color-warning);line-height:1.2;">' + formatCurrency(groupOutstanding) + '</div><div style="font-size:0.72rem;color:var(--text-secondary);line-height:1.3;">+' + formatCurrency(groupYield) + '/mo</div></div>';
    card.innerHTML = '<div style="display:flex;justify-content:space-between;align-items:center;"><div>' + hdrHtml;

    // Compact rows per loan
    group.loans.forEach(function(loan) {
      ...
      const settledBadge = stats.statusInMonth !== 'active' ? ' <span class="badge badge-muted">' + (loan.isEMI ? 'Closed' : 'Settled') + '</span>' : '';

      card.innerHTML += '<div style="border-top:1px solid var(--border-color);margin:0.35rem 0;"></div>';

      if (stats.statusInMonth !== 'active') {
        card.innerHTML += '<div style="font-size:0.82rem;"><span style="font-weight:600;">' + formattedPrincipal + settledBadge + '</span> <span style="color:var(--text-secondary);">· ' + recvDisplay + '</span></div>';
      } else if (loan.isEMI) {
        ...
        card.innerHTML += '<div style="font-size:0.72rem;color:#fff;font-style:italic;">' + recvDisplay + (stats.advTotal > 0 ? ' · Adv ' + formatCurrency(stats.advTotal) : '') + (stats.lastPaymentDate ? ' · ' + formatDate(stats.lastPaymentDate) : '') + '</div>';
      }
    });

    // Bottom bar: summary + single input + icon strip
    var safeId = allIds.replace(/,/g, '_');
    var primaryId = group.loans[0].id;
    var isPEMI = group.loans[0].isEMI;
    var isPActive = group.loans[0]._stats.statusInMonth === 'active';
    ...
    var icons =
      ...
      (isPActive ? (isPEMI ? '<span onclick="promptRecordEMI(\'' + primaryId + '\',\'received\')" title="Record EMI">📅</span>' : '<span onclick="quickReceiveInterest(\'' + primaryId + '\',\'lent\')" title="Quick Receive">⚡</span><span onclick="promptPayment(\'' + primaryId + '\',\'received\',\'principal\')" title="Repay">💰</span>') : '<span onclick="toggleLoanStatus(\'' + primaryId + '\',\'lent\')" title="Reopen">🔄</span>') +
      (isPActive ? '<span onclick="lendMore(\'' + primaryId + '\')" title="Lend More">➕</span>' : '') +
      (isPActive && !isPEMI ? '<span onclick="promptConvertEMI(\'' + primaryId + '\',\'lent\')" title="Convert to EMI">📊</span>' : '') +
