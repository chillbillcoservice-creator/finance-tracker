...
let currentLendingFilter = 'all'; // 'all', 'loans', 'emi'
...
function toggleVaultView() {
  ...
  var btn = document.getElementById('btn-vault-toggle');
  if (btn) btn.textContent = vaultVisible ? '📋' : '🔐';
  var vc = document.getElementById('vault-content');
  if (vc) vc.style.display = vaultVisible ? 'block' : 'none';
  if (vaultVisible) renderVault();
}

function toggleDiaryMode() {
  diaryModeActive = !diaryModeActive;
  var btn = document.getElementById('btn-diary-mode');
  if (btn) btn.textContent = diaryModeActive ? '📖' : '📖';
  refreshActiveTab();
}

function renderDiaryMode() {
  var containerId = currentTab === 'tenants' ? 'tenants-diary-content' : 'finances-diary-content';
  var mode = currentTab === 'tenants' ? 'rent' : 'interest';
  renderDiary(document.getElementById(containerId), mode);
}
...
window.toggleDiaryMode = toggleDiaryMode;
...
window.showAddLoanLent = showAddLoanLent;
window.showAddLoanBorrowed = showAddLoanBorrowed;
...
function quickMarkInterestPaid(loanId, type, amount, monthStr) {
  ...
  saveState();
  var _msg = '✓ Interest of ' + formatCurrency(amount) + ' recorded as ' + (type === 'received' ? 'collected' : 'paid') + ' for ' + (type === 'received' ? loan.borrowerName : loan.financierName) + '!';
  setTimeout(function() {
    alert(_msg);
    renderDashboard();
  }, 80);
...
function quickGroupPayment(safeId, direction) {
  ...
  if (!input) return;
  var totalAmount = Number(input.value);
  if (!totalAmount || totalAmount <= 0) { alert('Enter an amount'); return; }
  input.value = '';
  var ids = safeId.split('_');
  var loans = state[direction === 'lent' ? 'lent' : 'borrowed'];
  var groupLoans = [];
  for (var i = 0; i < loans.length; i++) {
    if (ids.indexOf(loans[i].id) !== -1) groupLoans.push(loans[i]);
  }
  ...
  if (remaining > 0) {
    ...
    if (cycleNowPaid) {
      isAdvance = true;
      var advanceMonth = nextMonthDate.slice(0, 7);
      for (var i = 0; i < interestLoans.length; i++) {
        if (remaining <= 0) break;
        var item = interestLoans[i];
        var alreadyAdv = state.interestPayments.filter(function(p) { return p.loanId === item.loan.id && p.type === type && p.category === 'interest' && p.date && p.date.startsWith(advanceMonth); }).reduce(function(s, p) { return s + Number(p.amount); }, 0) + payments.filter(function(p) { return p.loanId === item.loan.id && p.category === 'interest' && p.isAdvance; }).reduce(function(s, p) { return s + Number(p.amount); }, 0);
        var needed = Math.max(0, item.yieldAmt - alreadyAdv);
        if (needed <= 0) continue;
        var take = Math.min(needed, remaining);
      ...
      for (var i = 0; i < emiLoans.length; i++) {
        if (remaining <= 0) break;
        var item = emiLoans[i];
        var alreadyAdv = state.interestPayments.filter(function(p) { return p.loanId === item.loan.id && p.type === type && p.category === 'principal' && p.date && p.date.startsWith(advanceMonth); }).reduce(function(s, p) { return s + Number(p.amount); }, 0) + payments.filter(function(p) { return p.loanId === item.loan.id && p.category === 'principal' && p.isAdvance; }).reduce(function(s, p) { return s + Number(p.amount); }, 0);
        var needed = Math.max(0, item.emiAmt - alreadyAdv);
        if (needed <= 0) continue;
        var take = Math.min(needed, remaining);
        var isPartial = (alreadyAdv + take) < item.emiAmt;
  ...
  for (var i = 0; i < payments.length; i++) {
    var p = payments[i];
    var note = p.category === 'interest' ? (p.isAdvance ? 'Interest received [Advance]' : 'Interest received') : (p.isAdvance && p.isPartial ? 'Principal repayment [Advance][Partial]' : (p.isAdvance ? 'Principal repayment [Advance]' : (p.isPartial ? 'Principal repayment [Partial]' : 'Principal repayment')));
    state.interestPayments.push({
      id: 'p' + Math.random().toString(36).substr(2, 9),
      loanId: p.loanId,
      type: type,
...
function showRenewDialog(rental, renewal, callback) {
  var fromStr = renewal.dateStr;
  var toDate = new Date(renewal.date);
  ...
  overlay.style.cssText = 'opacity:0;pointer-events:auto;';
  document.body.appendChild(overlay);
  var box = document.createElement('div');
  box.style.cssText = 'background:var(--bg-secondary);border:1px solid var(--border-color);border-radius:var(--radius-lg);padding:1.5rem;max-width:360px;width:100%;box-shadow:0 32px 64px -16px var(--shadow-strong);transform:scale(0.95);transition:transform 0.3s var(--ease-spring);';
  overlay.appendChild(box);
  box.innerHTML = '<div style="font-size:0.95rem;color:var(--text-primary);margin-bottom:1.25rem;line-height:1.5;text-align:center;">Renew from <strong>' + fromStr + '</strong> to <strong>' + toStr + '</strong>?</div><div style="display:flex;gap:0.5rem;"><button id="rd-renew" style="flex:1;padding:0.6rem 0.8rem;background:var(--color-success);border:none;border-radius:var(--radius-sm);color:#000;font-weight:700;cursor:pointer;font-size:0.85rem;">Renew</button><button id="rd-change" style="flex:1;padding:0.6rem 0.8rem;background:var(--color-warning);border:none;border-radius:var(--radius-sm);color:#000;font-weight:700;cursor:pointer;font-size:0.85rem;">Change</button><button id="rd-cancel" style="flex:1;padding:0.6rem 0.8rem;background:var(--bg-tertiary);border:1px solid var(--border-color);border-radius:var(--radius-sm);color:var(--text-secondary);cursor:pointer;font-size:0.85rem;">Cancel</button></div>';
  requestAnimationFrame(function() { overlay.style.opacity = '1'; box.style.transform = 'scale(1)'; });
  function close() { overlay.remove(); }
  overlay.querySelector('#rd-renew').onclick = function() { close(); callback(null); };
  overlay.querySelector('#rd-change').onclick = function() {
    close();
    var input = prompt('Enter start date for next 11-month period (DD-MM-YYYY):');
    if (!input) return;
  ...
  overlay.querySelector('#rd-cancel').onclick = close;
  overlay.onclick = function(e) { if (e.target === overlay) close(); };
}

function renewRentalAgreement(rentalId) {
  loadState();
  var rental = state.rentals.find(function(r) { return r.id === rentalId; });
  if (!rental) return;
  var renewal = getNextRenewal(rental.startDate, rental.nextRenewalDate);
  if (!renewal) return;
  showRenewDialog(rental, renewal, function(customDate) {
    var base = customDate || renewal.date;
...
const VIEWS = {
  ...
  tenants: { title: 'Tenants', subtitle: 'Rental management — leases, payments, and tenant details.', render: renderTenantsDashboard },
  finances: { title: 'Finances', subtitle: 'Lending, borrowing, and interest tracking.', render: renderFinancesDashboard },
  records: { title: 'Vault', subtitle: 'Credentials, documents, construction, and settings.', render: renderRecords },
  expenses: { title: 'Expenses', subtitle: 'Track expenses, upload documents, manage construction.', render: renderExpensesDashboard }
};

function renderExpensesDashboard() {
  loadState();
  _recordsFilter = null;
  var todayExp = state.expenses.filter(function(e) { return e.date === selectedDateStr && !isFinalisedConst(e); }).reduce(function(s, e) { return s + (Number(e.amount) || 0); }, 0);
  var expNode = document.getElementById('dash-total-expenses');
  if (expNode) expNode.textContent = formatCurrency(todayExp);
  var weekStart = new Date(selectedDateStr);
  ...
  var weekEndStr = weekEnd.getFullYear() + '-' + pad(weekEnd.getMonth()+1) + '-' + pad(weekEnd.getDate());
  var weekExp = state.expenses.filter(function(e) { return e.date >= weekStartStr && e.date <= weekEndStr && !isFinalisedConst(e); }).reduce(function(s, e) { return s + (Number(e.amount) || 0); }, 0);
  var monthExp = state.expenses.filter(function(e) { return e.date.startsWith(selectedMonthStr) && !isFinalisedConst(e); }).reduce(function(s, e) { return s + (Number(e.amount) || 0); }, 0);
  var wNode = document.getElementById('dash-weekly-expenses');
  if (wNode) wNode.innerHTML = 'This Week: <span style="color: var(--color-danger); font-weight: 800;">' + formatCurrency(weekExp) + '</span>';
  var mNode = document.getElementById('dash-monthly-expenses');
  if (mNode) mNode.innerHTML = 'This Month: <span style="color: var(--color-danger); font-weight: 800;">' + formatCurrency(monthExp) + '</span>';
  var detailNode = document.getElementById('expense-card-details');
  if (detailNode) {
    var todayItems = state.expenses.filter(function(e) { return e.date === selectedDateStr && !isFinalisedConst(e); }).slice(0, 5);
    if (todayItems.length > 0) {
      var h = '';
      todayItems.forEach(function(exp) {
        var txt = exp.note || exp.category;
        if (exp.category === 'construction') txt = 'Construction - ' + (exp.laborType || 'General') + ' - ' + (exp.project || '');
        h += '<div style="display: flex; align-items: center; padding: 0.2rem 0.4rem; background: var(--input-bg); border-radius: 4px; font-size: 0.7rem;">' +
          '<span style="flex: 1; font-weight: 600; color: var(--text-primary);">' + txt + '</span>' +
          '<span style="flex: 1; text-align: right; font-weight: 700; color: var(--color-danger);">' + formatCurrency(exp.amount) + '</span></div>';
      });
      detailNode.innerHTML = h;
...
function renderTenantsDashboard() {
  loadState();
  var normal = document.getElementById('tenants-normal');
  var diaryContainer = document.getElementById('tenants-diary-container');
  if (diaryModeActive) {
...
function renderFinancesDashboard() {
  loadState();
  var normal = document.getElementById('finances-normal');
  var diaryContainer = document.getElementById('finances-diary-container');
  if (diaryModeActive) {
    if (normal) normal.style.display = 'none';
    if (diaryContainer) diaryContainer.style.display = '';
    renderDiaryMode();
...
function renderDiary(container, mode) {
  loadState();
  var diaryEl = container;
  if (!diaryEl) return;
  ...
  if (!mode || mode === 'rent' || mode === 'all') {
    ...
    state.rentals.forEach(function(r) {
      if (r.startDate <= monthStr + '-31' && r.status === 'active') {
        ...
        lines.push('  ' + num + '. ' + r.tenantName + extra);
        var suffix = collected >= due ? ' ✓' : '';
        if (pending > 0) lines.push('      ' + formatCurrency(due) + suffix + '  Due: ' + formatCurrency(pending));
        else lines.push('      ' + formatCurrency(due) + suffix);
      }
    });
  }
  
  // ── INTEREST RECEIVED ──
  if (!mode || mode === 'interest' || mode === 'all') {
    var num = 0;
    lines.push('');
  ...
  diaryEl.textContent = lines.join('\n');
}
...
function setLendingFilter(mode) {
  if (currentLendingFilter === mode) mode = 'all';
  currentLendingFilter = mode;
  ['all', 'loans', 'emi'].forEach(function(m) {
    var el = document.getElementById('lending-filter-' + m);
...
function switchTab(tabId) {
  currentTab = tabId;
  window.scrollTo(0, 0);
  var vaultBtn = document.getElementById('btn-vault-toggle');
  if (vaultBtn) vaultBtn.style.display = (tabId === 'records') ? '' : 'none';
  ...
  if (inlineCal) {
    inlineCal.style.display = (tabId === 'dashboard' || tabId === 'tenants' || tabId === 'finances') ? 'block' : 'none';
  }
  var monthBar = document.getElementById('month-selector-bar');
  var yearBar = document.getElementById('year-selector-bar');
  if (tabId === 'tenants' || tabId === 'finances') {
...
function isFinalisedConst(e) { return e && e.category === 'construction' && state && state.projectStatus && state.projectStatus[e.project] === 'finalised'; }
function renderDashboard() {
  ...
  const selectedDayExpenses = state.expenses
    .filter(p => p.date === selectedDateStr && !isFinalisedConst(p))
  ...
  const selectedWeekExpenses = state.expenses
    .filter(p => p.date >= weekStartStr && p.date <= weekEndStr && !isFinalisedConst(p))
    .reduce((sum, exp) => sum + Number(exp.amount), 0);

  const selectedMonthExpenses = state.expenses
    .filter(p => p.date.startsWith(selectedMonthStr) && !isFinalisedConst(p))
    .reduce((sum, exp) => sum + Number(exp.amount), 0);
  ...
  if (expenseCardDetails) {
    if (state.showExpenseDetails !== false) {
      const todayExps = state.expenses.filter(function(e) { return e.date === selectedDateStr && !isFinalisedConst(e); }).slice(0, 5);
    ...
    var constExps = state.expenses.filter(function(e) { return e && e.category === 'construction' && !isFinalisedConst(e) && e.date && e.date.startsWith(selectedMonthStr); });
    var projectTotals = {};
    ...
    var activeProjects = Object.keys(projectTotals).filter(function(p) { return projectTotals[p] > 0; });
    if (activeProjects.length > 0) {
      ...
      activeProjects.forEach(function(p) {
        ...
        projHtml += '<div style="cursor: pointer; font-size: 0.78rem; font-weight: 600; color: var(--color-accent); padding: 0.15rem 0; display: flex; justify-content: space-between; align-items: center;" onclick="event.stopPropagation(); switchTab(\'expenses\'); setTimeout(function(){ selectRecordsTab(\'construction\', \'' + p.replace(/'/g, "\\'") + '\'); }, 100);"><span>🏗 ' + p + '</span><span style="color: var(--color-danger); font-weight: 700;">' + formatCurrency(total) + '</span></div>';
      });
      projHtml += '</div>';
  ...
  if (state.showPendingNames !== false && !isDayMode && !isYearMode) {
      ...
      state.rentals.forEach(function(r) {
        if (r.startDate <= endDateOfSelectedMonth && r.status === 'active') {
          var dueDay = Math.min(r.rentDueDay, new Date(todayBase.getFullYear(), todayBase.getMonth() + 1, 0).getDate());
          var dueDate = new Date(todayBase.getFullYear(), todayBase.getMonth(), dueDay);
          dueDate.setHours(0, 0, 0, 0);
          ...
          if (!isPaid && diffDays < 0) {
            ...
            var weight = '600';
            if (diffDays <= -10) { color = 'var(--color-danger)'; weight = '800'; }
            else if (diffDays <= -7) { color = 'var(--color-warning)'; weight = '700'; }
            pTenants.push({name: r.tenantName, amount: r.monthlyRent, color: color, weight: weight});
          }
        }
      });
  ...
  activeRentals.forEach(rental => {
    ...
    const renewalInfo = getNextRenewal(rental.startDate, rental.nextRenewalDate);
    if (renewalInfo) {
...
function renderLending() {
  ...
  var allLoans = state.lent.filter(l => l.startDate <= endDateOfSelectedMonth);
  var visibleLoans = allLoans;
  if (currentLendingFilter === 'loans') visibleLoans = allLoans.filter(l => !l.isEMI);
  else if (currentLendingFilter === 'emi') visibleLoans = allLoans.filter(l => l.isEMI);

  // Outstanding totals (always show overall stats in header)
  ...
  var regOutstanding = allLoans.filter(function(l) { return !l.isEMI; }).reduce(function(s, l) { return s + calcOutstanding(l); }, 0);
  var emiOutstanding = allLoans.filter(function(l) { return l.isEMI; }).reduce(function(s, l) { return s + calcOutstanding(l); }, 0);
  var emiCount = allLoans.filter(function(l) { return l.isEMI; }).length;
  var nonEmiCount = allLoans.length - emiCount;
  if (countEl) countEl.textContent = allLoans.length;
  if (regPrincipalEl) regPrincipalEl.textContent = formatCurrency(regOutstanding);
  const emiCountEl = document.getElementById('lent-emi-count');
  if (emiCountEl) emiCountEl.textContent = emiCount;
  if (emiTotalEl) emiTotalEl.textContent = emiCount > 0 ? formatCurrency(emiOutstanding) : '';
  if (totalEl) totalEl.textContent = formatCurrency(regOutstanding + emiOutstanding);
  if (visibleLoans.length === 0) {
    var emptyMsg = 'No active lending records for this period.';
    if (currentLendingFilter === 'loans') emptyMsg = 'No non-EMI loans for this period.';
    else if (currentLendingFilter === 'emi') emptyMsg = 'No EMI loans for this period.';
    listContainer.innerHTML = `
      <div class="empty-state">
  ...
  visibleLoans.forEach(loan => {
    ...
    const isInterestFullyPaidThisMonth = monthlyYield > 0 && currentMonthInterestSum >= (monthlyYield - 0.01);
    ...
    const advanceEmiPayments = principalPayments.filter(p => p.note && p.note.indexOf('[Advance]') !== -1);
    const advanceEmiCount = loan.isEMI ? advanceEmiPayments.length : 0;
    const advEmiAmount = loan.isEMI ? advanceEmiPayments.reduce(function(s, p) { return s + Number(p.amount); }, 0) : 0;
    ...
    loan._stats = {
      ...
      emiTotalCount, emiPaidCount, partialEmiTotal, advanceEmiCount, advEmiAmount, advInterest
    };
  });
  ...
  // Group all loans by borrower name
  var groups = {};
  sortedLoans.forEach(function(l) { var n = l.borrowerName || 'Unknown'; if (!groups[n]) groups[n] = []; groups[n].push(l); });
  var groupNames = Object.keys(groups).sort();

  groupNames.forEach(function(name) {
    var loans = groups[name];
    var first = loans[0];
    var allSimple = loans.every(function(l) { return !l.isEMI; });
    var allEMI = loans.every(function(l) { return l.isEMI; });

    if (allSimple) {
      // Type 1: Simple Loan — individual cards
      loans.forEach(function(loan) {
        ...
        var html = '<div style="display:flex;justify-content:space-between;align-items:center;">' +
          ...
          (loan.phone ? '<div style="font-size:0.82rem;color:#fff;margin-top:0.05rem;">' + loan.phone + '</div>' : '') + '</div>' +
          '<div style="text-align:right;"><div style="font-size:1.15rem;font-weight:800;color:var(--color-warning);line-height:1.2;">' + formattedPrincipal + '</div><div style="font-size:0.72rem;color:var(--text-secondary);line-height:1.3;">+' + formattedYield + '/mo</div></div></div>';

        if (stats.statusInMonth === 'active') {
          html += '<div style="display:flex;gap:0.35rem;align-items:center;margin-bottom:0.25rem;margin-top:0.35rem;">' +
        ...
        html += '<div class="icon-strip"><div class="icon-strip-left">' +
          ...
          (stats.statusInMonth === 'active' ? '<span onclick="lendMore(\'' + loan.id + '\')" title="Lend More">➕</span><span onclick="promptPayment(\'' + loan.id + '\',\'received\',\'principal\')" title="Record Principal">➖</span><span onclick="promptConvertEMI(\'' + loan.id + '\',\'lent\')" title="Convert to EMI">📊</span>' : '<span onclick="toggleLoanStatus(\'' + loan.id + '\',\'lent\')" title="Reopen">🔄</span>') +
        '</div><div class="icon-strip-right"><span onclick="editLoan(\'' + loan.id + '\',\'lent\')" title="Edit">✏️</span><span onclick="deleteLoan(\'' + loan.id + '\',\'lent\')" title="Delete">🗑️</span></div></div>';
        ...
        card.setAttribute('data-loan-ids', loan.id);
        card.innerHTML = html;
        card.onclick = function(e) {
          if (e.target.closest('[onclick]') || e.target.closest('button') || e.target.closest('input')) return;
          showLedger(loan.id, 'lent');
        };
        listContainer.appendChild(card);
      });
    } else if (allEMI) {
      // Type 3: EMI Only — grouped card
      ...
      var leftIcons =
        ...
        (isPActive ? '<span onclick="lendMore(\'' + primaryId + '\')" title="Lend More">➕</span><span onclick="promptPayment(\'' + primaryId + '\',\'received\',\'principal\')" title="Record Principal">➖</span>' : '<span onclick="toggleLoanStatus(\'' + primaryId + '\',\'lent\')" title="Reopen">🔄</span>');
      var rightIcons =
      ...
      card.onclick = function(e) {
        if (e.target.closest('[onclick]') || e.target.closest('button') || e.target.closest('input')) return;
        showLedger(primaryId, 'lent');
      };
      listContainer.appendChild(card);
    } else {
      // Type 2: Loan + EMI — grouped card with mixed loans (non-EMI first, then EMI)
      var sortedGroup = loans.slice().sort(function(a, b) {
        if (a._stats.statusInMonth !== b._stats.statusInMonth) return a._stats.statusInMonth === 'active' ? -1 : 1;
      ...
      var allIds = sortedGroup.map(function(l) { return l.id; }).join(',');
      var groupOutstanding = 0, groupYield = 0, totalRcvd = 0;
      sortedGroup.forEach(function(l) {
        groupOutstanding += l._stats.outstandingPrincipal;
        groupYield += l._stats.monthlyYield;
        totalRcvd += l._stats.currentMonthSum;
      });
      ...
      var isPActive = sortedGroup[0]._stats.statusInMonth === 'active';
      var phone = first.phone ? first.phone.replace(/\D/g, '') : '';
      var convertEmiId = null;
      for (var j = 0; j < sortedGroup.length; j++) {
        if (sortedGroup[j]._stats.statusInMonth === 'active' && !sortedGroup[j].isEMI) {
          convertEmiId = sortedGroup[j].id; break;
        }
      }
      var leftIcons =
        (phone ? '<span onclick="window.open(\'tel:' + phone + '\',\'_self\')" title="Call">📞</span><span onclick="window.open(\'https://wa.me/91' + phone + '\',\'_blank\')" title="WhatsApp">💬</span>' : '') +
        (isPActive ? '<span onclick="lendMore(\'' + primaryId + '\')" title="Lend More">➕</span><span onclick="promptPayment(\'' + primaryId + '\',\'received\',\'principal\')" title="Record Principal">➖</span>' + (convertEmiId ? '<span onclick="promptConvertEMI(\'' + convertEmiId + '\',\'lent\')" title="Convert to EMI">📊</span>' : '') : '<span onclick="toggleLoanStatus(\'' + primaryId + '\',\'lent\')" title="Reopen">🔄</span>');
      var rightIcons =
        '<span onclick="editLoan(\'' + primaryId + '\',\'lent\')" title="Edit">✏️</span>' +
        '<span onclick="deleteLoan(\'' + primaryId + '\',\'lent\')" title="Delete">🗑️</span>';
...
function renderBorrowing() {
  ...
  // Always group by financier name
  const loanGroupsB = [];
  const map = {};
  ...
  loanGroupsB.forEach(function(group) {
    const allIds = group.loans.map(function(l) { return l.id; }).join(',');
    ...
    card.innerHTML = `
      ...
      <div style="font-size:0.68rem;color:#fff;margin:0.15rem 0 0.25rem;">
        ${rental.status === 'active'
          ? `<span style="color:var(--color-warning);font-weight:700;">DUE</span>: ${rental.rentDueDay}<sup>${getOrdinal(rental.rentDueDay)}</sup> · Since ${formatDate(rental.startDate)}${renewData ? ` · Renews: ${renewData.dateStr}` : ''}${!isRentPaidThisMonth ? '<span style="color:var(--color-warning);font-weight:600;float:right;">Due' + (carryOverArrears > 0 ? ' · Bal: ' + formatCurrency(carryOverArrears) : '') + '</span>' : ''}`
      ...
      <div class="icon-strip">
        <div class="icon-strip-left">
          ...
          <span onclick="renewRentalAgreement('${rental.id}')" title="Renew Agreement">📋</span>
          ${rental.status === 'active'
            ? `<span onclick="var r=document.getElementById('quick-rent-row-${rental.id}');if(r)r.style.display=r.style.display==='none'?'':'none';" title="Pay">💰</span><span onclick="deductFromSecurity('${rental.id}')" title="Deduct from Security">🔐</span><label style="display:inline-flex;align-items:center;justify-content:center;width:28px;height:28px;cursor:pointer;"><input type="checkbox" onchange="if(this.checked){quickMarkRentPaid('${rental.id}')}else{quickUnmarkRentPaid('${rental.id}')}" ${isRentFullyPaid ? 'checked' : ''} style="width:16px;height:16px;accent-color:var(--color-success);          cursor:pointer;margin:2px 0 0 0;"></label>`
...
function promptConvertEMI(loanId, direction) {
  ...
  // Match card calculation: principal + topups - repayments
  const loanPayments = state.interestPayments.filter(p => p.loanId === loan.id && p.type === 'received');
  const totalRepaid = loanPayments.filter(p => p.category === 'principal').reduce((s, p) => s + Number(p.amount), 0);
  const totalTopups = loanPayments.filter(p => p.category === 'increase').reduce((s, p) => s + Number(p.amount), 0);
  const outstandingPrincipal = Math.max(0, Number(loan.principal) + totalTopups - totalRepaid);
  
  var pendingInterest = 0;
  var totalAmount = outstandingPrincipal;
  var outstandingM = getOutstandingPrincipalAtMonth(loan.id, loan.principal, selectedMonthStr);
  if (outstandingM > 0 && Number(loan.interestRate) > 0) {
    var expected = outstandingM * (Number(loan.interestRate) / 100);
    var paid = state.interestPayments.filter(function(p) {
      return p.type === 'received' && p.category === 'interest' && p.loanId === loan.id && p.date.startsWith(selectedMonthStr) && (!p.note || p.note.indexOf('[Advance]') === -1);
    }).reduce(function(s, p) { return s + Number(p.amount); }, 0);
    pendingInterest = Math.max(0, expected - paid);
    totalAmount = outstandingPrincipal + pendingInterest;
  }
  ...
  document.getElementById('emi-convert-amount').value = totalAmount;
  ...
  var noteEl = document.getElementById('emi-convert-note');
  if (noteEl) {
    if (pendingInterest > 0) {
      noteEl.innerHTML = 'Principal <span style="color:var(--color-warning);font-weight:600">' + formatCurrency(outstandingPrincipal) + '</span> + Pending Interest <span style="color:var(--color-accent);font-weight:600">' + formatCurrency(pendingInterest) + '</span>';
...
document.getElementById('form-emi-convert').addEventListener('submit', (e) => {
  ...
  // Match card calculation: principal + topups - repayments
  const loanPmts = state.interestPayments.filter(p => p.loanId === loan.id && p.type === 'received');
  const totalRepaidP = loanPmts.filter(p => p.category === 'principal').reduce((s, p) => s + Number(p.amount), 0);
  const totalTopupsP = loanPmts.filter(p => p.category === 'increase').reduce((s, p) => s + Number(p.amount), 0);
  const outstandingP = Math.max(0, Number(loan.principal) + totalTopupsP - totalRepaidP);
  
  // Calculate pending interest (for all-in-one EMI conversion)
  var pendingInt = 0;
  var outstandingM = getOutstandingPrincipalAtMonth(loan.id, loan.principal, selectedMonthStr);
  if (outstandingM > 0 && Number(loan.interestRate) > 0) {
    var expectedM = outstandingM * (Number(loan.interestRate) / 100);
    var paidM = state.interestPayments.filter(function(p) {
      return p.type === 'received' && p.category === 'interest' && p.loanId === loan.id && p.date.startsWith(selectedMonthStr) && (!p.note || p.note.indexOf('[Advance]') === -1);
    }).reduce(function(s, p) { return s + Number(p.amount); }, 0);
    pendingInt = Math.max(0, expectedM - paidM);
  }
  
  var totalEffectivePrincipal = outstandingP + pendingInt;
  
  if (!emiPrincipal || emiPrincipal <= 0) { alert('Enter a valid amount to convert.'); return; }
  if (emiPrincipal > totalEffectivePrincipal) { alert('Amount exceeds outstanding principal + pending interest of ' + formatCurrency(totalEffectivePrincipal)); return; }
  ...
  // Record pending interest as paid (rolled into principal)
  if (pendingInt > 0) {
    var todayPI = new Date().toISOString().split('T')[0];
    state.interestPayments.push({
      id: 'p' + Math.random().toString(36).substr(2, 9),
      loanId: loanId,
      type: direction === 'lent' ? 'received' : 'paid',
      ...
      amount: pendingInt,
      date: todayPI,
      note: 'Interest rolled into principal (EMI conversion)'
    });
  }
  
  if (emiPrincipal >= totalEffectivePrincipal) {
    // Full conversion — increase principal by pending interest
    if (pendingInt > 0) loan.principal = Number(loan.principal) + pendingInt;
...
function showLedger(loanId, direction) {
  ...
  // Set Title
  var name = direction === 'lent' ? loan.borrowerName : loan.financierName;
  var phone = loan.phone || '';
  var callHtml = phone ? '<span onclick="event.stopPropagation();window.open(\'tel:' + phone.replace(/\D/g, '') + '\',\'_self\')" style="cursor:pointer;font-size:0.75rem;" title="Call">📞</span><span onclick="event.stopPropagation();window.open(\'https://wa.me/91' + phone.replace(/\D/g, '') + '\',\'_blank\')" style="cursor:pointer;font-size:0.75rem;margin-left:0.15rem;" title="WhatsApp">💬</span>' : '';

  // Group totals for title (match grouped card sums)
  var groupOutstanding = 0, groupYield = 0;
  list.forEach(function(l) {
    if (l.borrowerName === name && l.phone === loan.phone) {
      var inc = state.interestPayments.filter(function(p) { return p.loanId === l.id && p.type === type && p.category === 'increase'; }).reduce(function(s, p) { return s + Number(p.amount); }, 0);
      var rep = state.interestPayments.filter(function(p) { return p.loanId === l.id && p.type === type && p.category === 'principal'; }).reduce(function(s, p) { return s + Number(p.amount); }, 0);
      var rem = Math.max(0, Number(l.principal) + inc - rep);
      groupOutstanding += rem;
      groupYield += l.isEMI ? Number(l.emiAmount) : rem * (Number(l.interestRate) / 100);
    }
  });
  var showGroup = groupOutstanding > 0 && list.filter(function(l) { return l.borrowerName === name && l.phone === loan.phone; }).length > 1;
  ...
  const topupHistory = history.filter(p => p.category === 'increase');
  ...
  const totalIncreases = topupHistory.reduce((sum, p) => sum + Number(p.amount), 0);
  const remainingPrincipal = Math.max(0, Number(loan.principal) + totalIncreases - totalPrincipalRepaid);
  var monthlyYield = direction === 'lent' ? (loan.isEMI ? Number(loan.emiAmount) : remainingPrincipal * (Number(loan.interestRate) / 100)) : 0;
  var displayPrincipal = showGroup ? groupOutstanding : remainingPrincipal;
  var displayYield = showGroup ? groupYield : monthlyYield;
  var subHtml = formatCurrency(displayPrincipal) + (displayYield > 0 ? '<br>+' + formatCurrency(displayYield) + '/mo' : '');
  document.getElementById('ledger-title').innerHTML = '<span style="display:flex;align-items:center;gap:0.7rem;flex-wrap:wrap;">' + name + '<span style="display:inline-flex;align-items:center;gap:0.15rem;">' + callHtml + '</span></span><span style="font-size:0.78rem;color:var(--text-primary);font-weight:500;display:block;margin-top:0.15rem;">' + subHtml + '</span>';
...
function toggleRentalStatus(id) {
  ...
  if (rental.status === 'active') {
    if (!confirm('Do you want to end this lease?')) return;
...
window.renewRentalAgreement = function(rentalId) {
  var rental = state.rentals.find(function(r) { return r.id === rentalId; });
  if (!rental) return;
  var renewal = getNextRenewal(rental.startDate, rental.nextRenewalDate);
  if (!renewal) return;
  showRenewDialog(rental, renewal, function(customDate) {
    ...
    saveState();
    renderRentals();
    showToast('Rent agreement renewed!', 'success');
  });
};
...
window.loadRealSeedData = function() {
  ...
  loadState();
  // Auto-backup ONLY if there's real data to preserve
  var hasRealData = (state.lent && state.lent.length > 0) || (state.borrowed && state.borrowed.length > 0) || (state.rentals && state.rentals.length > 0);
  if (hasRealData) {
    try {
  ...
  months.forEach(function(month) {
    ...
    if (month === '2026-07') {
      ...
      state.rentPayments.push({id:'rp_seed_' + (pid++),rentalId:'r8',amount:15000,monthYear:'2026-07',datePaid:'2026-07-20',note:'Rent - Jul 2026 (adjusted from security)'});
    }
  });
...
window.finaliseProject = function(name) {
  if (!confirm('Finalise "' + name + '"? All current expenses will be archived and total reset to 0.')) return;
  state.projectStatus[name] = 'finalised';
  state.projectArchivedTotals = state.projectArchivedTotals || {};
  var archTotal = 0;
  state.expenses.forEach(function(e) {
    if (e && e.category === 'construction' && e.project === name && !e.archived) {
      e.archived = true;
      e.archivedDate = new Date().toISOString().slice(0, 10);
      archTotal += Number(e.amount) || 0;
    }
  });
  state.projectArchivedTotals[name] = (state.projectArchivedTotals[name] || 0) + archTotal;
  saveState();
  renderConstruction();
};

window._constHistoryExpanded = {};
window.toggleConstHistoryRows = function(id) {
  var el = document.getElementById('const-history-rows-' + id);
  var btn = document.getElementById('const-history-toggle-' + id);
  if (!el || !btn) return;
  var expanded = el.style.display !== 'none';
  el.style.display = expanded ? 'none' : 'flex';
  btn.textContent = expanded ? '+' : '−';
  window._constHistoryExpanded[id] = !expanded;
};

window.showConstructionHistory = function() {
  var archivedExps = (state.expenses || []).filter(function(e) { return e && e.category === 'construction' && e.archived; });
  var grouped = {};
  archivedExps.forEach(function(e) {
    if (!grouped[e.project]) grouped[e.project] = [];
    grouped[e.project].push(e);
  });
  var projectNames = Object.keys(grouped).sort();
  var html = '';
  if (projectNames.length === 0) {
    html = '<div style="padding: 2rem; text-align: center; color: var(--text-muted);">No archived construction history.</div>';
  } else {
    projectNames.forEach(function(proj) {
      var safeId = proj.replace(/[^a-zA-Z0-9]/g, '_');
      if (window._constHistoryExpanded[safeId] === undefined) window._constHistoryExpanded[safeId] = false;
      var exps = grouped[proj].sort(function(a, b) { return new Date(a.date || 0) - new Date(b.date || 0); });
      var total = exps.reduce(function(s, e) { return s + (Number(e.amount) || 0); }, 0);
      var firstDate = exps[0].date;
      var lastDate = exps[exps.length - 1].archivedDate || exps[exps.length - 1].date;
      html += '<div style="margin-bottom: 1rem; border: 1px solid var(--border-color); border-radius: 8px; padding: 0.75rem; background: var(--bg-card);">' +
        '<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem;">' +
          '<strong style="color: var(--color-accent);">✅ ' + proj + '</strong>' +
          '<div style="display:flex;align-items:center;gap:0.5rem;">' +
            '<span style="font-weight: 700; color: var(--color-danger);">' + formatCurrency(total) + '</span>' +
            '<button id="const-history-toggle-' + safeId + '" onclick="toggleConstHistoryRows(\'' + safeId + '\')" style="background:none;border:1px solid var(--border-color);border-radius:4px;cursor:pointer;padding:0.1rem 0.35rem;font-size:0.85rem;color:var(--text-secondary);line-height:1;transition:transform 0.2s;">' + (window._constHistoryExpanded[safeId] ? '−' : '+') + '</button>' +
          '</div>' +
        '</div>' +
        '<div style="font-size: 0.7rem; color: var(--text-secondary); margin-bottom: 0.5rem;">' + formatDate(firstDate) + ' → ' + formatDate(lastDate) + '</div>' +
        '<div id="const-history-rows-' + safeId + '" style="display: ' + (window._constHistoryExpanded[safeId] ? 'flex' : 'none') + '; flex-direction: column; gap: 0.25rem;">';
      exps.forEach(function(exp) {
        html += '<div style="display: flex; justify-content: space-between; align-items: center; padding: 0.3rem 0.5rem; background: var(--input-bg); border-radius: 4px; font-size: 0.75rem;">' +
          '<div>' +
            '<span style="font-weight: 600; color: var(--text-primary);">' + (exp.laborType || 'General') + '</span>' +
            '<span style="color: var(--text-secondary); margin-left: 0.5rem;">' + formatDate(exp.date) + (exp.note ? ' - ' + exp.note : '') + '</span>' +
          '</div>' +
  ...
  var contentEl = document.getElementById('construction-history-content');
  if (contentEl) contentEl.innerHTML = html;
  openModal('modal-construction-history');
};
...
function renderConstruction() {
  ...
  try {
    ...
    var unarchivedExps = selectedProject ? constructionExpenses.filter(function(e) { return e && e.project === selectedProject && !e.archived; }).sort(function(a, b) { return new Date(b.date || 0) - new Date(a.date || 0); }) : [];
    var allExpsSorted = selectedProject ? constructionExpenses.filter(function(e) { return e && e.project === selectedProject; }).sort(function(a, b) { return new Date(b.date || 0) - new Date(a.date || 0); }) : [];
    var total = unarchivedExps.reduce(function(sum, e) { return sum + (Number(e.amount) || 0); }, 0);
    ...
    var expensesHtml = '';
    if (unarchivedExps.length > 0) {
      unarchivedExps.slice(0, 10).forEach(function(exp) {
    ...
    for (var i = 0; i < allProjects.length; i++) {
      ...
      projectBtnsHtml += '<button class="btn btn-sm" data-project="' + p + '" onclick="selectConstProject(this)" style="flex:1; min-width:80px; padding:0.3rem; font-size:0.7rem; background:' + (isSel ? (isFinalised ? 'var(--color-purple)' : 'var(--color-accent)') : 'var(--bg-secondary)') + '; color:' + (isSel ? '#fff' : (isFinalised ? 'var(--text-secondary)' : 'var(--text-primary)')) + '; border:1px solid ' + (isSel ? (isFinalised ? 'var(--color-purple)' : 'var(--color-accent)') : 'var(--border-color)') + '; cursor:pointer;">' + p + '</button>';
    }
    ...
    // Build summary card project rows
    var projectsWithExpenses = allProjects.filter(function(p) {
      return constructionExpenses.filter(function(e) { return e && e.project === p && !e.archived; }).reduce(function(s, e) { return s + (Number(e.amount) || 0); }, 0) > 0;
    });
    var countEl = document.getElementById('count-construction');
    if (countEl) countEl.textContent = allProjects.length;
    
    var summaryProjectsEl = document.getElementById('construction-summary-projects');
    if (summaryProjectsEl) {
      var summaryHtml = '';
      var ongoingCount = 0;
      projectsWithExpenses.forEach(function(p) {
        var isFinalised = state.projectStatus[p] === 'finalised';
        if (!isFinalised) { ongoingCount++; }
        var unarchivedTotal = constructionExpenses.filter(function(e) { return e && e.project === p && !e.archived; }).reduce(function(s, e) { return s + (Number(e.amount) || 0); }, 0);
        var archivedTotal = state.projectArchivedTotals && state.projectArchivedTotals[p] ? state.projectArchivedTotals[p] : 0;
        var icon = isFinalised ? '✅' : '🏗';
        var labelStyle = isFinalised ? 'color:var(--text-secondary);opacity:0.7;' : 'color:var(--text-primary);';
        ...
        summaryHtml += '<div style="display:flex;justify-content:space-between;align-items:center;padding:0.15rem 0.25rem;font-size:0.8rem;font-weight:600;white-space:nowrap;overflow:hidden;' + labelStyle + '">' +
          '<span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex-shrink:1;min-width:0;font-size:0.8rem;">' + icon + ' ' + p + '</span>' +
          '<span style="' + amtStyle + 'font-weight:700;font-size:0.8rem;">' + amtDisplay + '</span>' +
          finaliseBtn +
        '</div>';
      });
      summaryProjectsEl.innerHTML = summaryHtml;
    }
    
    var subtextEl = document.getElementById('subtext-construction');
    if (subtextEl) {
      var historyCount = 0;
      allProjects.forEach(function(p) { if (state.projectArchivedTotals && state.projectArchivedTotals[p] > 0) historyCount++; });
      var subParts = [];
      if (historyCount > 0) subParts.push(historyCount + ' History');
...
function updateFileSummaryCards() {
  ['bills', 'documents', 'policies'].forEach(function(fileType) {
    ...
    var vaultCountEl = document.getElementById('vault-count-' + fileType);
    if (vaultCountEl) vaultCountEl.textContent = files.length;
  });
}
...
function initAppListeners() {
  ...
  var btnLoanLent = document.getElementById('btn-add-loan-lent');
  if (btnLoanLent) btnLoanLent.addEventListener('click', showAddLoanLent);

  var btnLoanBorrowed = document.getElementById('btn-add-loan-borrowed');
  if (btnLoanBorrowed) btnLoanBorrowed.addEventListener('click', () => {
    document.getElementById('form-loan').reset();
    document.getElementById('loan-id').value = '';
    document.getElementById('loan-direction').value = 'borrowed';
    document.getElementById('loan-rate').value = '3.00';
  ...
  // Add rentals trigger
  var btnAddRental = document.getElementById('btn-add-rental');
  if (btnAddRental) btnAddRental.addEventListener('click', () => {
    document.getElementById('form-rental').reset();
    document.getElementById('rental-id').value = '';
    document.getElementById('rental-start-date').value = new Date().toISOString().split('T')[0];
...
window.openExpenseDetails = function(mode, event) {
  ...
  const todayExps = state.expenses.filter(e => e.date === todayStr && !isFinalisedConst(e));
  ...
  const weekExps = state.expenses.filter(e => e.date >= startStr && e.date <= endStr && !isFinalisedConst(e));
  const weekTotal = weekExps.reduce((sum, e) => sum + Number(e.amount), 0);
  
  const monthExps = state.expenses.filter(e => e.date.startsWith(selectedMonthStr) && !isFinalisedConst(e));
  ...
  const allExps = !mode ? [...state.expenses].filter(function(e) { return !isFinalisedConst(e); }).sort((a, b) => new Date(b.date) - new Date(a.date)) : filteredExpenses;
...
window.openTenantDetails = function(rentalId) {
  ...
  var renewBtn = renewData && renewData.daysLeft <= 30 ? '<button onclick="renewRentalAgreement(\'' + rental.id + '\')" style="padding:0.2rem 0.4rem;font-size:0.6rem;background:var(--color-warning);border:none;border-radius:var(--border-radius-sm);cursor:pointer;color:#000;font-weight:700;white-space:nowrap;">Renew Agreement</button>' : '';
  titleEl.innerHTML = '<span style="display:flex;align-items:center;gap:0.5rem;">' + rental.tenantName + '<span class="contact-btn-group" style="display:inline-flex;align-items:center;gap:0.3rem;">' + callLink + waLink + '</span>' + renewBtn + '</span>';
...
window.markPendingCollected = function(collectionType, itemType, ids, amount, monthStr) {
  loadState();
  const today = new Date().toISOString().split('T')[0];
  var pairList = ids ? ids.toString().split(',') : [ids];
  pairList.forEach(function(pair) {
    var parts = pair.split(':');
    var id = parts[0];
    var amt = parts.length > 1 ? Number(parts[1]) : Number(amount) / pairList.length;
    if (itemType === 'rent') {
      const paymentId = 'rp' + Math.random().toString(36).substr(2, 9);
      state.rentPayments.push({ id: paymentId, rentalId: id, amount: amt || Number(amount), monthYear: monthStr, datePaid: today, note: 'Marked Paid from Collection' });
    } else {
      const paymentId = 'p' + Math.random().toString(36).substr(2, 9);
...
window.window.openCollectionDetails = function(type, event) {
  ...
  if (type === 'rent') {
    ...
    collected = Object.keys(groupedCol).map(name => {var r = state.rentals.find(x => x.id === groupedIds[name][0]); var rd = r ? getNextRenewal(r.startDate, r.nextRenewalDate) : null; var d = rd && rd.date; return {name, amount: groupedCol[name], ids: groupedIds[name], type: 'rent', propertyName: r ? r.propertyName : '', renewalDate: d ? ('0' + d.getDate()).slice(-2) + '/' + ('0' + (d.getMonth() + 1)).slice(-2) + '/' + d.getFullYear() : '', renewalDue: rd ? rd.daysLeft <= 30 : false};});
    ...
    if (!isDayMode && !isYearMode) {
      state.rentals.forEach(r => {
        if (r.startDate <= endDateOfSelectedMonth && r.status === 'active') {
          ...
          if (pOwe > 0) {var rd = getNextRenewal(r.startDate, r.nextRenewalDate); var d = rd && rd.date; pending.push({name: r.tenantName, phone: r.contactInfo, owe: pOwe, id: r.id, type: 'rent', propertyName: r.propertyName, renewalDate: d ? ('0' + d.getDate()).slice(-2) + '/' + ('0' + (d.getMonth() + 1)).slice(-2) + '/' + d.getFullYear() : '', renewalDue: rd ? rd.daysLeft <= 30 : false});}
        }
      });
    }
    ...
    const rawPayments = state.interestPayments.filter(function(p) { if (p.type !== 'received' || !filterPayment(p)) return false; if (p.category === 'issuance' || p.category === 'increase') return false; if (p.category === 'principal') { var _pl = state.lent.find(function(x) { return x.id === p.loanId; }); return _pl && _pl.isEMI; } return true; });
    const groupedCol = {};
    const groupedIds = {};
    const groupedAdv = {};
    rawPayments.forEach(p => {
    ...
    if (!isDayMode && !isYearMode) {
      ...
      activeLendingLoans.forEach(l => {
        ...
        if (outstanding > 0) {
          ...
          var catFilter = function(p) { return (p.category === 'interest' || (p.category === 'principal' && l.isEMI)); };
          const pPaid = state.interestPayments.filter(p => p.type === 'received' && p.loanId === l.id && p.date.startsWith(selectedMonthStr) && (!p.note || p.note.indexOf('[Advance]') === -1) && catFilter(p)).reduce((sum, p) => sum + Number(p.amount), 0);
    ...
    // Merge duplicate names in interest section
    var mergedMap = {};
    pending.forEach(function(p) {
      if (mergedMap[p.name]) {
        mergedMap[p.name].owe += p.owe;
        mergedMap[p.name].id += ',' + p.id + ':' + p.owe;
        if (p.dueDate && (!mergedMap[p.name].dueDate || p.dueDate < mergedMap[p.name].dueDate)) { mergedMap[p.name].dueDate = p.dueDate; mergedMap[p.name].isOverdue = p.isOverdue; }
      } else {
        mergedMap[p.name] = {name: p.name, phone: p.phone, owe: p.owe, id: p.id + ':' + p.owe, dueDate: p.dueDate, isOverdue: p.isOverdue, type: 'interest'};
      }
    });
    pending = Object.keys(mergedMap).map(function(name) { return mergedMap[name]; });
  } else if (type === 'all') {
    ...
    if (!isDayMode && !isYearMode) {
      ...
      activeLendingLoans.forEach(l => {
        ...
        if (outstanding > 0) {
          const expected = l.isEMI ? Number(l.emiAmount || 0) : outstanding * (Number(l.interestRate) / 100);
          var catFilter2 = function(p) { return (p.category === 'interest' || (p.category === 'principal' && l.isEMI)); };
          const pPaid = state.interestPayments.filter(p => p.type === 'received' && p.loanId === l.id && p.date.startsWith(selectedMonthStr) && (!p.note || p.note.indexOf('[Advance]') === -1) && catFilter2(p)).reduce((sum, p) => sum + Number(p.amount), 0);
          const pOwe = expected - pPaid;
          if (pOwe > 0) {
            const normName = (l.borrowerName || '').toLowerCase().trim();
            const groupId = 'group-' + btoa(encodeURIComponent(normName)).replace(/[^a-zA-Z0-9]/g, '');
    ...
    // Merge duplicate interest names in all section
    var mergedMap2 = {};
    pending.forEach(function(p) {
      if (p.type === 'interest') {
        if (mergedMap2[p.name]) {
          mergedMap2[p.name].owe += p.owe;
          mergedMap2[p.name].id += ',' + p.id + ':' + p.owe;
          if (p.dueDate && (!mergedMap2[p.name].dueDate || p.dueDate < mergedMap2[p.name].dueDate)) { mergedMap2[p.name].dueDate = p.dueDate; mergedMap2[p.name].isOverdue = p.isOverdue; }
        } else {
          mergedMap2[p.name] = {name: p.name, owe: p.owe, id: p.id + ':' + p.owe, dueDate: p.dueDate, isOverdue: p.isOverdue, type: 'interest', phone: p.phone};
        }
      }
    });
    Object.keys(mergedMap2).forEach(function(name) {
      pending = pending.filter(function(p) { return p.type !== 'interest' || p.name !== name; });
      pending.push(mergedMap2[name]);
    });
  }
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // ==========================================
  const tabsOrder = ['tenants', 'finances', 'dashboard', 'records', 'settings'];
  let touchStartX = 0;
  let touchEndX = 0;
  let touchStartY = 0;
  let touchEndY = 0;
...
function renderGlanceWidget() {
  ...
  function setGlanceDetails(html) {
    var sep = ' <span onclick="event.stopPropagation();dismissGlanceRenewal()" style="cursor:pointer;margin-left:0.5rem;opacity:0.5;font-size:0.82rem;">\u2715<\/span>';
    var idx = html.indexOf(sep);
    if (idx > 0) {
      var txt = html.substring(0, idx);
      detailsEl.innerHTML = '<span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' + txt + '</span><span onclick="event.stopPropagation();dismissGlanceRenewal()" style="cursor:pointer;flex-shrink:0;white-space:nowrap;opacity:0.5;font-size:0.82rem;margin-left:0.15rem;">\u2715<\/span>';
    } else {
      detailsEl.innerHTML = '<span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' + html + '</span>';
    }
